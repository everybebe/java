...
public class LprodMybatisTest {
	...
	/*	
		...
		try {
			...
			//SqlSession객체 변수를 이용하여 처리할 SQL문을 호출해서 실행한다.
	...
	*/	
	...
	/*	
		...
		lvo.setLprod_gu(lprodGu2);
		lvo.setLprod_id(lprodId2);
		lvo.setLprod_nm(lprodNm2);
	...
	*/
	...
	/*	
		...
		try {
			...
			//SqlSession객체 변수를 이용하여 처리할 SQL문을 호출해서 실행한다.
			//select문의 처리 결과가 여러개일 경우에 selectlist()메소드를 사용하는데
			//이 메소드는 여러개의 레 코드 각각을 VO객체에 담은 후 이 VO객체를 List에 추가해 주는
			//작업을 자동으로 수행한다.
			//형식) session.selectList("namespace속성값.id속성값", 파라미터클래스)
			//		반환값 : resultType에서 지정한 객체가 저장된 List객체
	...
	*/
		...
		try {
			...
			//SqlSession객체 변수를 이용하여 처리할 SQL문을 호출해서 실행한다.
			//select문의 처리 결과가 1개일 경우에는 selectOne()메소드를 사용한다.
			//형식) session.seletOn("namespace속성값:id속성값", 파라미터클래스)
