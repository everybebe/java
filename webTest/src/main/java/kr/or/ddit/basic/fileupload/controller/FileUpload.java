...
/*
  - Servlet 3.0이상에서 파일업로드를 하려면 작성하는 서블릿에 @MutipartConfig 애노테이션을 설정해야 한다.
  ...
  4) maxRequestSize : 서버로 전송되는 전체 Request 데이터의 전체 최대 크기(단위 : byte, 기본값 : -1L(무제한))
...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//파일이 아닌 일반 데이터는 getParameter()메소드나 getParameterValues()메소드를 이용해서 구한다.
		String userName = request.getParameter("username");
		System.out.println("일반 파리미터 데이터 : " + userName);
		...
		/*
		 ...
		 1) request.getParts() ==> 전체 Part객체를 Collection객체에 담아서 반환하다.
		...
		//List에 저장된 파일 정보를 DB에 insert 한다.
	...
	/*
	 - Part 구조
	 1) 파일이 아닌 일반 데이터일 경우
	 -------------12sdfjaslkfasf23242 ==> Part를 구분하는 구분선
	 ...
	 contetn-type : text/plain          => 파일의 종류
	  	       		==> 빈줄
	 abcd123안녕     ==> 파일의 내용
